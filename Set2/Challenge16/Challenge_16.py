...
def first(userinput : bytes) -> bytes:
    prefix = b"comment1=cooking%20MCs;userdata="
    suffix = b";comment2=%20like%20a%20pound%20of%20bacon"
    userinput = prefix+userinput.replace(b";", b"_").replace(b"=", b"_")+suffix
    return aes_cbc_encrypt(pkcs7_pad(userinput, 16), key, iv)

def second(cyphertext : bytes) -> bytes:
    plain = strip_pkcs7_pad(aes_cbc_decrypt(cyphertext, key, iv))
    print(plain)
    for i in plain.split(b';'):
        if i.split(b'=') == [b'admin', b'true']:
            return True
    return False
...
payload = b'!admin3true'
cypher = first(payload)
cypher = bytearray(cypher)

cypher[16] = cypher[16] ^ ord('!') ^ ord(b';')
cypher[22] = cypher[22] ^ ord('3') ^ ord(b'=')
print(second(cypher))
